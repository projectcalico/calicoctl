...
const (
	...
	AUTOD_METHOD_FIRST     = "first-found"
	AUTOD_METHOD_CAN_REACH = "can-reach="
	AUTOD_METHOD_INTERFACE = "interface="
)
...
Options:
  -h --help                Show this screen.
     ...
     --ip-autodetection-method=<IP_AUTODETECTION_METHOD>
                           ...
                           local IPv6 routing address for this node.  The valid
                           options are:
                           > first-found
                             Use the first valid IP address on the first
                             enumerated interface (filtering out common known
                             exceptions such as the docker bridge).  It is not
...
func validateIpAutodetectionMethod(method string, version int) {
	if method == AUTOD_METHOD_FIRST {
		...
		os.Exit(1)
	} else if strings.HasPrefix(method, AUTOD_METHOD_INTERFACE) {
		// Auto-detection method is "interface", validate that the interface
		// regex is a valid golang regex.
		ifStr := strings.TrimPrefix(method, AUTOD_METHOD_INTERFACE)
