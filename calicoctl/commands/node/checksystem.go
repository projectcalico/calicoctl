...
func Checksystem(args []string) error {
	doc := `Usage:
  <BINARY_NAME> node checksystem [--filename=<FILENAME>]

Options:
  -h --help                 Show this screen.
  -f --filename=<filename>	Override kernel bootfile location, checksystem
							can use this file if config file is not present
...
`
	...
	if parsedArgs["--filename"] != nil {
		overrideBootFile = parsedArgs["--filename"].(string)
	}
...
func findBootFile(kernelVersion string) string {
	// If user has provided an override boot file skip default locations.
	if overrideBootFile != "" {
		return overrideBootFile
	}

	// default locations that we will look for bootconfig file.
